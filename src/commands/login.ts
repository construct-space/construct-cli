...
async function loginWithToken(token: string): Promise<void> {
  // Catch obvious token-format mismatches before the round-trip so the
  // user gets pointed at the right flow instead of a generic "rejected".
  if (token.startsWith('cst_live_')) {
    console.error(chalk.red('cst_live_* tokens are being retired.'))
    console.error(chalk.dim('  Sign in via the Construct desktop app, or get a new'))
    console.error(chalk.dim(`  identity token at ${chalk.cyan('https://my.construct.space/settings/tokens')}.`))
    process.exit(1)
  }
  if (token.startsWith('csk_live_')) {
    console.error(chalk.red("That's a publisher key, not an identity token."))
    console.error(chalk.dim('  Publisher keys (csk_live_*) authorize publish-as-publisher'))
    console.error(chalk.dim('  on top of identity. Pass an identity token (cat_*) here;'))
    console.error(chalk.dim('  the CLI loads your publisher key automatically once logged in.'))
    process.exit(1)
  }
