...
export async function scaffold(nameArg?: string, options?: ScaffoldOptions): Promise<void> {
  ...
  // Create directories. The space root IS an agentskills.io skill folder
  // (SKILL.md + scripts/ + references/ + assets/), so the standard
  // sub-directories live at the top alongside src/, agent/, widgets/.
  const dirs = [
    name,
    join(name, 'src', 'pages'),
    join(name, 'src', 'components'),
    join(name, 'src', 'composables'),
    // agentskills.io skill anatomy
    join(name, 'scripts'),
    join(name, 'references'),
    join(name, 'assets'),
    join(name, 'tools'),
    join(name, 'lib'),
    // Construct-side agent config (hooks + custom tools) -- separate
  ...
  const files: Record<string, string> = {
    ...
    'style.css.tmpl': join(name, 'src', 'style.css'),
    'index.vue.tmpl': join(name, 'src', 'pages', 'index.vue'),
    'config.md.tmpl': join(name, 'agent', 'config.md'),
    // SKILL.md goes at the space root per agentskills.io -- name field
    // matches the parent dir (the space id).
    'skill.md.tmpl': join(name, 'SKILL.md'),
    ...
    'construct.md.tmpl': join(name, 'CONSTRUCT.md'),
  ...
  // Full preset -- extra pages + reference docs the SKILL.md links to.
  // Reference docs are *not* separate skills; they're loaded on demand
  // by the agent when SKILL.md tells it to. One skill per space.
  if (isFull) {
    writeTemplate(templateDir, 'full/settings.vue.tmpl', join(name, 'src', 'pages', 'settings.vue'), data)
    writeTemplate(templateDir, 'full/skill-data.md.tmpl', join(name, 'references', 'data.md'), data)
    console.log(chalk.blue('Full preset: settings page + references/data.md'))
  }
