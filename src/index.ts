...
program
  ...
  .option('--scope <scope>', "Publish as 'user' (personal) or 'org'. Without this flag, the active publisher in auth.json decides.")
  .option('--api-key <key>', 'Publisher API key (csk_live_...). Overrides any key stored in the active profile. Also reads CONSTRUCT_PUBLISHER_KEY.')
...
// Org command group -- focused on "what am I publishing AS?" view, which
// is the question whoami doesn't quite answer (it shows identity, not the
// publisher credential CLI will attach on the next publish).
const org = program
  .command('org')
  .description('Inspect the active organization context')

org
  .command('status')
  .description('Show active scope, publisher, and roles')
  .action(async () => (await import('./commands/org.js')).orgStatus())
...
space.command('publish').option('-y, --yes').option('--bump <type>').option('--private').option('--public').option('--api-key <key>')
  .action(async (opts) => publish(opts))
