...
type WidgetKind = 'Widget' | 'TvWidget'
...
function resolveWidgetList(widgets: SpaceManifest['widgets'], prefix: string, kind: WidgetKind): WidgetImport[] {
  const imports: WidgetImport[] = []
  for (const w of widgets || []) {
    for (const sizeKey of Object.keys(w.sizes).sort()) {
      imports.push({
        // `kind` keeps home and TV var names distinct so a space declaring both
        // (same id/size) doesn't collide on a single import identifier.
        varName: capitalize(w.id) + kind + sizeKey,
...
function resolveWidgets(m: SpaceManifest, prefix: string): WidgetImport[] {
  return resolveWidgetList(m.widgets, prefix, 'Widget')
}

function resolveTvWidgets(m: SpaceManifest, prefix: string): WidgetImport[] {
  return resolveWidgetList(m.tv, prefix, 'TvWidget')
}

/** Emit a nested `id -> { sizeKey -> VarName }` object literal for a widget set. */
function emitWidgetMap(lines: string[], key: string, widgets: WidgetImport[]): void {
  lines.push(`  ${key}: {`)
...
export function generate(root: string, m: SpaceManifest): string {
  ...
  const tvWidgets = resolveTvWidgets(m, '../')
  ...
  if (existsSync(join(root, 'src', 'style.css'))) {
  ...
  for (const w of tvWidgets) lines.push(`import ${w.varName} from '${w.importPath}'`)
  ...
  if (widgets.length > 0) emitWidgetMap(lines, 'widgets', widgets)
  if (tvWidgets.length > 0) emitWidgetMap(lines, 'tvWidgets', tvWidgets)
