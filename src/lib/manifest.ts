...
export interface SpaceManifest {
  ...
  widgets?: Widget[]
  /**
   * TV-optimized widget variants (10-foot UI, remote/D-pad driven). Same shape
   * as `widgets`, but the size paths point under `tv/` instead of `widgets/`.
   * The TV host prefers these over the matching home widget; falls back to it
   * when a TV variant for that id/size is absent. Purely additive — omit on
   * spaces with no TV presence.
   */
  tv?: Widget[]
